document.addEventListener("DOMContentLoaded", () => {
  ...
  const benefitsData = {
    peso: {
      ...
      desc: "Acelera el metabolismo y ayuda a quemar grasas, promoviendo la pérdida de peso de forma natural."
    },
    azucar: {
      title: "Control de Azúcar",
      desc: "Mejora la secreción de insulina y la utilización de glucosa en la sangre."
    },
    corazon: {
      title: "Salud Cardiovascular",
      desc: "Rico en grasas saturadas naturales que aumentan el colesterol bueno (HDL) en el cuerpo."
    },
    digestion: {
      title: "Mejora la Digestión",
      desc: "Ayuda al cuerpo a absorber vitaminas, minerales y aminoácidos esenciales."
    },
    dental: {
      title: "Protección Dental",
      desc: "Mata bacterias causantes de caries y mejora la salud de las encías y la placa dental."
    },
    apetito: {
      title: "Reduce el Apetito",
      desc: "Los triglicéridos de cadena media (MCT) inducen una mayor sensación de saciedad."
    },
    inmune: {
      title: "Sistema Inmune",
      desc: "El ácido láurico y la monolaurina protegen contra virus, hongos y bacterias patógenas."
    },
    pelo: {
      title: "Cabello y Piel",
      desc: "Actúa como crema hidratante y protector solar natural, previniendo daños capilares."
    },
    cerebral: {
      title: "Función Cerebral",
      desc: "Los ácidos grasos proporcionan energía rápida al cerebro y mejoran la memoria."
    },
    microbiana: {
      title: "Acción Antimicrobiana",
      desc: "Ayuda a combatir infecciones de manera efectiva gracias a sus propiedades antifúngicas."
    }
  };

  const benefitItems = document.querySelectorAll('.benefit-item');
  const coconutImg = document.querySelector('.central-coconut');
  const centralPanel = document.querySelector('.central-panel');
  const panelTitle = document.getElementById('panel-title');
  const panelDesc = document.getElementById('panel-desc');
  const allLines = document.querySelectorAll('.line-path');

  benefitItems.forEach(item => {
    const benefitType = item.getAttribute('data-benefit');
    
    item.addEventListener('mouseover', () => {
      if(coconutImg) coconutImg.style.opacity = '0';
      if(centralPanel) centralPanel.classList.remove('hidden');
      if(panelTitle) panelTitle.textContent = benefitsData[benefitType].title;
      if(panelDesc) panelDesc.textContent = benefitsData[benefitType].desc;
      
      const targetLine = document.querySelector(`.line-path.${benefitType}`);
      if(targetLine) {
    ...
    item.addEventListener('mouseout', () => {
      if(coconutImg) coconutImg.style.opacity = '1';
